(function() {
  ...
  // Zmienna do przechowywania pozycji etykiet (by wykrywać kolizje)
  let cellLabelsPositions = [];

  function showTableCellsOverlays() {
    cellLabelsPositions = []; // resetujemy przed rysowaniem
    ...
    cells.forEach(cell => {
      ...
      // Po dodaniu do DOM, zmierzmy boundingRect etykiety
      // i jeśli się z czymś pokrywa, przesuwajmy w dół, aż nie będzie kolizji
      function measureLabel() {
        return label.getBoundingClientRect();
      }

      let labelRect = measureLabel();
      let shiftAttempt = 0;
      const maxShifts = 50; // ograniczenie, by nie wpaść w pętlę

      // Prosta funkcja sprawdzająca kolizję (overlap) dwóch rectów
      ...
      // Dopóki etykieta koliduje z istniejącymi etykietami, przesuwajmy w dół
      let collision = true;
      while (collision && shiftAttempt < maxShifts) {
        collision = false;
        for (const existing of cellLabelsPositions) {
          if (overlaps(labelRect, existing)) {
            collision = true;
            // przesuwamy label w dół o 12px
            label.style.top = (label.offsetTop + 12) + 'px';
            // Zaktualizujmy labelRect
            labelRect = measureLabel();
            shiftAttempt++;
            break; // wyjdź z pętli for i sprawdź ponownie
          }
        }
      }

      // Gdy już nie ma kolizji, zapiszmy pozycję w tablicy
      cellLabelsPositions.push(labelRect);
    });
  }
